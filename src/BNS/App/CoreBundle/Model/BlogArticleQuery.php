...
class BlogArticleQuery extends BaseBlogArticleQuery
{
	...
	public static function getArticlesFromBlog(Blog $blog, &$pager = null, $page = null, $filters = null, $limit = 5)
	{
			...
			if (isset($filters['filters']) && count($filters['filters']) > 0) {
				foreach ($filters['filters'] as $filter) {
					if ('programmed' == $filter) {
						$query->addOr(BlogArticlePeer::PUBLISHED_AT, time(), Criteria::GREATER_THAN);
					}
					else if (BlogArticlePeer::STATUS_PUBLISHED_INTEGER == $filter) {
						$query->addOr(BlogArticlePeer::PUBLISHED_AT, time(), Criteria::LESS_EQUAL);
					}
					else {
						$query->addOr(BlogArticlePeer::STATUS, $filter);
					}
				}
			}
		...
		$query->addAnd(BlogArticlePeer::BLOG_ID, $blog->getId());
	...
	{
		$this->addAnd(BlogArticlePeer::STATUS, BlogArticlePeer::STATUS_PUBLISHED_INTEGER);
		$this->addAnd(BlogArticlePeer::PUBLISHED_AT, time(), Criteria::LESS_EQUAL);
