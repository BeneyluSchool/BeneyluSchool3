...
class TranslationGenerator extends Generator
{
	...
	public function generate($bundleName, $className, $languages, $attributes)
	{
		...
		foreach ($languages as $language) {
			$filePath = __DIR__ . '/../Resources/translations/' . $bundleName . 'Bundle/' . strtoupper(Container::underscore($className)) . '.' . $language . '.xliff';
			if (file_exists($filePath)) {
				throw new RuntimeException(sprintf('Unable to generate the translation as the target file "%s" is not empty.', $filePath));
			}
		...
		foreach ($filePathes as $filePath) {
			$this->renderFile($this->skeletonDir, 'messages.xliff', $filePath, $parameters);
		}
	}
}