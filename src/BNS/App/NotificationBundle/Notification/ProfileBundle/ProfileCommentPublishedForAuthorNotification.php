...
/**
 * Notification generation date : 28/01/2013 18:36:16 
 */
class ProfileCommentPublishedForAuthorNotification extends Notification implements NotificationInterface
{
	const NOTIFICATION_TYPE = 'PROFILE_COMMENT_PUBLISHED_FOR_AUTHOR';
	...
	public static function translate(Notification $notification, $objects)
	{
		...
		$userManager = self::$container->get('bns.user_manager');
		...
		$finalObjects['%link_word%']		= $userManager->setUser($notification->getUser())->isAdult() ? 'vos' : 'tes';
		$finalObjects['%sender_full_name%'] = $comment->getAuthor()->getFullName();
		$finalObjects['%comment_route%']	= $notification->getBaseUrl() . self::$container->get('cli.router')->generate('profile_feed_visualisation', array(
