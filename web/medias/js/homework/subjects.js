...
$(function ()
{
    $('.group-filter-list').click (function ()
    {
		var thisCheck = $(this).find('.group-filter');
		...
		$.ajax(
		{
			url: Routing.generate('BNSAppHomeworkBundle_backajax_group_session', {
				'groupId': groupId, 
				'add': add
			}),
			type: 'POST',
		...
		$('.day-filter-list').click (function ()
		{
			var thisCheck = $(this).find('.day-filter');
			...
			$('#loading-layer').show();
			$(this).parent().find('.loader').fadeIn('fast');
			...
			$.ajax(
			{
				url: Routing.generate('BNSAppHomeworkBundle_backajax_day_session', {
					'dayId': dayId, 
					'add': add
				}),
				type: 'POST',
    ...
    $('body').on('click', '.homework-subjects-filter li', function (e)
    {
		...
		$.ajax({
			url: Routing.generate('BNSAppHomeworkBundle_backajax_subject_session', {
				'subjectId': id, 
				'add': $checkbox.hasClass('checked') ? 1 : 0
			}),
			type: 'POST',
			success: function (data)
			{
