...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        var __SCOLLEVENTTEMP = "<DIV style=\"cursor:pointer; WIDTH:${width};top:${top};left:${left};\" class=\"chip chip${i} ${drag} ${agendaclass} ${agendacolor}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}\" class=ct>&nbsp;</DIV><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=\"${recurrentclass}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\"></DT><DD><span class=\"event-time\" style='font-size: 10px;'>${starttime} - ${endtime}<br /></span><SPAN class=\"event-title\">${content}</SPAN></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; BORDER-RIGHT-COLOR: ${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb1>&nbsp;</DIV><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BORDER-TOP-COLOR:${bdcolor}; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb2>&nbsp;</DIV></DIV>";
        var __ALLDAYEVENTTEMP = '<div class="rb-o ${eclass} ${agendaclass} ${agendacolor}" id="${id}" style="cursor:pointer; color:${color};"><div class="dhdV" style="display:none">${data}</div><div class="${extendClass} rb-m ${recurrentclass}">${extendHTML}<div class="rb-i">${content}</div></div></div>';
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
            ...
            var offset = (endHour - startHour) * hourCellHeight;
            ...
            html.push("<table style=\"height:"+ offset +"px\" id=\"tgTable\" class=\"tg-timedevents\" cellspacing=\"0\" cellpadding=\"0\"><tbody>");
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            var tabClassDay = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
        ...
        function BuildDayScollEventContainer(ht, dayarrs, events) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                // Today
                if (istoday) {
                	var offset = (endHour - startHour + 1) * hourCellHeight;
                    ht.push("<div style=\"margin-bottom: -"+ offset +"px; height:"+ offset + "px\" class=\"tg-today\">&nbsp;</div>");
                }
                //var eventC = $(eventWrap);
                //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
                ht.push("<div  style=\"margin-bottom: -"+ offset +"px; height:"+ offset + "px\" id='tgCol", i, "' class=\"tg-col-eventwrapper\">");
        ...
        function gP(h, m) {
            return (h - startHour) * hourCellHeight + parseInt(m / 60 * hourCellHeight);
        }
        function gW(ts1, ts2) {
            var t1 = (ts1) / 42;
            var t2 = parseInt(t1);
...
//            }
        ...
        function buildtempdayevent(sh, sm, eh, em, h, title, w, resize, thindex, isDragEvent) {
            ...
            var newtemp = Tp(__SCOLLEVENTTEMP, {
                ...
                bgcolor1: theme[2],
                data: "",
                starttime: [pZero(sh), pZero(sm)].join("h"),
                endtime: [pZero(eh), pZero(em)].join("h"),
                content: title ? title : i18n.xgcalendar.new_event,
                title: title ? title : i18n.xgcalendar.new_event,
                icon: "<I class=\"\">&nbsp;</I>",
                top: (isDragEvent? "0" : startHour * 42) + "px",
                left: "",
                width: w ? w : "100%",
                height: h - 4,
                i: "-1",
        ...
        function dayupdate(data, start, end) {
            if (option.quickUpdateUrl != "" && data[8] == 1 && option.readonly != true) {
                ...
                var od = data[3];
                var zone = new Date().getTimezoneOffset() / 60 * -1;
                var param = [{ "name": "event_slug", value: id },
							{ "name": "datetime_start", value: dateFormat.call(start, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm") },
							{ "name": "datetime_end", value: dateFormat.call(end, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm") },
							//{ "name": "timezone", value: zone }
						   ];
                var d;
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            // FIXME
            /*var timezone = (start.getTimezoneOffset() * 60) * -1;
            var dtstart = (start.getTime() / 1000);// + (timezone > 0? ;
            var dtend = (end.getTime() / 1000);// + timezone;*/
            var link = Routing.generate('BNSAppReservationBundle_back_add_event', {'start': start, 'end': end, 'allday': (isallday === true? 1 : 0)});
                ...
                switch (d.type) {
                    case 1:
                        ...
                        if (diffy > 11 || diffy < -11 || d.cpwrap) {
                            ...
                            var tempdata;
                            if (!d.cpwrap) {
                                tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh, gh.em, gh.h, false);
                            ...
                            if (diffx > 5 || diffx < -5 || diffy > 5 || diffy < -5 || d.cpwrap) {
                                ...
                                if (!d.cpwrap) {
                                   ...
                                   d.target.hide();
                                    ny = gP(gh.sh, gh.sm);
                                    d.top = ny;
                                    tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh, gh.em, gh.h, data[1], false, false, data[7], true);
                                    var cpwrap = $("<div class='ca-evpi drag-chip-wrapper' style='top:" + ny + "px'/>").html(tempdata);
                        ...
                        if (data != null && data[8] == 1) {
                            ...
                            if (diffy != 0 || d.cpwrap) {
                                ...
                                if (!d.cpwrap) {
                                    ...
                                    d.target.hide();
                                    ny = gP(gh.sh - startHour, gh.sm);
                                    d.top = ny;
                                    // /!\
                                    tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh, gh.em, gh.h, data[1], "100%", true, data[7], false);
                                    var cpwrap = $("<div class='ca-evpi drag-chip-wrapper' style='top:" + ny + "px'/>").html(tempdata);
                                    var evid = d.target.parent().attr("id").replace("tgCol", "#tgOver");
                                    $(evid).append(cpwrap);
                                    d.cpwrap = cpwrap;
                                    ...
                                    if (d.nh != nh) {
                                        ...
                                        tempdata = buildtempdayevent(gh.sh + startHour, gh.sm, gh.eh + startHour, gh.em, gh.h, data[1], "100%", true, data[7], false);
                ...
                switch (d.type) {
                    case 1: //day view
                        ...
                        if (!d.cpwrap) {
                            ...
                            var ny = gP(gh.sh, gh.sm);
                            var tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh, gh.em, gh.h, false);
                        ...
                        start = (start.getTime() / 1000) + (startHour * 60 * 60);
                        end = (end.getTime() / 1000) + (startHour * 60 * 60);
                    ...
                    case 4: // event moving
                        if (d.cpwrap) {
                            ...
                            start.setHours(gh.sh + startHour, gh.sm);
                            end.setHours(gh.eh + startHour, gh.em);
                    ...
                    case 5: //Resize
                        if (d.cpwrap) {
                            ...
                            var offset = startHour * hourCellHeight;
                            var gh = gW(d.top - offset, d.top + nh - offset);
                            //Tweak Eric : +24 pour compenser l'offset de -1 jour
                            gh.sh = gh.sh + 24;
                            gh.eh = gh.eh + 24;
